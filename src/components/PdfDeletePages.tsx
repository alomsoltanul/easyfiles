...
export default function PdfDeletePages() {
  ...
  const { isProcessing, result, error, process, download, reset } = usePdfTool({
    ...
    options: { pages: Array.from(selectedPages) },
  });
            ...
            <>
              <div className="border-t border-slate-100 pt-6">
                <div className="flex items-center justify-between mb-3">
                  <h2 className="text-lg font-bold text-slate-800">Select Pages to Delete</h2>
                  ...
                  {thumbnails.map((thumb, i) => {
                    ...
                    return (
                      <button key={i} onClick={() => togglePage(page)}
                        className={`relative rounded-xl border-2 overflow-hidden transition-all ${isSelected ? 'border-red-500 shadow-sm shadow-red-200' : 'border-slate-200 opacity-60 hover:opacity-80'}`}>
                        <img src={thumb} alt={`Page ${page}`} className="w-full h-auto" />
                        <span className={`absolute top-1.5 left-1.5 text-[10px] font-bold px-1.5 py-0.5 rounded ${isSelected ? 'bg-red-500 text-white' : 'bg-slate-800 text-white'}`}>{page}</span>
                        {isSelected && (
              ...
              <div className="border-t border-slate-100 pt-6">
                <button onClick={handleProcess} disabled={isProcessing || selectedPages.size === 0}
                  className="w-full bg-red-500 hover:bg-red-600 disabled:bg-red-300 text-white font-bold py-3.5 rounded-xl transition-all shadow-sm shadow-red-200">
          ...
          {result && (
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-800">Pages Deleted!</h2>
              ...
              <button onClick={handleReset} className="w-full py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl">Delete More Pages</button>
            </div>
          )}
        </>
      )}
